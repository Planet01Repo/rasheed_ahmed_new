...
class OrderStatusCustomerExport implements FromView
{
    private $data;
    ...
    public function view(): View
    {
        ...
        // if(isset($id)){
        //     $data = FacadesDB::select("select customers.name, perfoma_invoices.perfoma_invoice_no_local, perfoma_invoices.po_number, products.accepted_date, sizes.name as size_name, products.name as product_name, perfoma_invoice_details.quantity order_quantity, perfoma_invoice_details.unit,
        ...
        //     JOIN sizes on perfoma_invoice_details.size_id = sizes.id where perfoma_invoices.customer_id = {$id}");
        // dd($data);
        $data = $this->data;
        return view('superadmin.reports-excel.order-status-customer-excel', ['data' => $data]);
        // }
    }
}